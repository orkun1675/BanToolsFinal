...
public class UnLockBan implements CommandExecutor {
	...
	public UnLockBan(BanToolsFinal main, MysqlManager mysql) {
		this.main = main;
		this.mysql = mysql;
	}
    ...
    public boolean onCommand(CommandSender sender, Command cmnd, String label, String[] args) {
		if (args.length == 1) {
			...
			if (!mysql.isBanLocked(args[0])) {
				main.sendMessage(sender, "ban_not_locked", new String[] {});
				return true;
			}
			mysql.unLockBan(args[0]);
			main.sendMessage(sender, "unlockban.inform", new String[] {args[0]});
			return true;
		}
		return false;
	}
